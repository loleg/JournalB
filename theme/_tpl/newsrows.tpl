...
{{ if ($start + $articles_num_1) < $articles_num_total_1 or ($start2 + $articles_num_2) < $articles_num_total_2 }}
	<div class="weitere" style="clear:both">
		<div class="wline"></div>
		<a href="{{ url options="section" }}?ls-art0={{ $start + $articles_num_1 }}&ls-art1={{ $start2 + $articles_num_2 }}&time={{ php }}echo time();{{ /php }}" onclick="return loadWeitereArtikel(this);">weitere Artikel</a>
	</div>
{{ /if }}
