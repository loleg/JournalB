...
final class MetaSubtitle {
    ...
    public static function ProcessImageLink(array $p_matches) {
		...
		$imgString = (strlen($imgZoomLink) > 0) ? $imgZoomLink : '';
        $uri->uri_parameter = "image $imageOptions";
		
        $isCentered = false;
        $imgString .= '<img src="'.$uri->uri.'" class="cs_img '.$detailsArray['align'].'"';
		
        if (isset($detailsArray['align'])) 
		{
			if ($detailsArray['align'] == 'middle')
			{
				$imgString = '<div align="center">'.$imgString;
				$isCentered = true;
			}
			else
			{
        ...
        $imgString .= ' border="0"/>';
        $imgString .= (strlen($imgZoomLink) > 0) ? '</a>' : '';
		...
		if (isset($detailsArray['alt']) && !empty($detailsArray['alt'])) {
			$alt = str_replace("&lt;","<",$detailsArray['alt']);
			$alt = str_replace("&gt;",">",$alt);
            $imgString .= '<span class="imagedescription '.$detailsArray['align'].'">'.$alt.'</span>';
        }
